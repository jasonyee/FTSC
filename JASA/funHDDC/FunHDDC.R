...
# Functions
EncapFunHDDC <- function(dataset, n_cl, n_b, modeltype, init_cl){
  T = nrow(dataset)
  basis <- create.bspline.basis(c(0, T), nbasis=n_b)
  var1<-smooth.basis(argvals=seq(0,1,length.out = T),y=t(data_set[,1:T]),fdParobj=basis)$fd
  fdobj <- smooth.basis(1:T, dataset,basis,
                        fdnames=list("Time", "Subject", "Score"))$fd
  res = funHDDC(fdobj,n_cl,model=modeltype,init=init_cl, thd = 0.001)
